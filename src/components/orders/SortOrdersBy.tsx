...
const SortOrdersBy: React.FC = () => {
  const [selected, setSelected] = useState(sort[0]);
  const { setFilterOrders, orders } = useStore();
  ...
  useEffect(() => {
    if (selected === "All") {
      ...
      setFilterOrders(
        orders.filter((o: { status: string }) => o.status === "Delivered")
      );
    } else if (selected === "Cancelled") {
      setFilterOrders(
        orders.filter((o: { status: string }) => o.status === "Cancelled")
      );
    } else if (selected === "Mac only") {
      setFilterOrders(
        orders.filter((o: { category: string }) => o.category === "Laptop")
      );
    } else if (selected === "Iphone only") {
      setFilterOrders(
        orders.filter((o: { category: string }) => o.category === "Phone")
      );
    } else if (selected === "Ipad only") {
      setFilterOrders(
        orders.filter((o: { category: string }) => o.category === "Ipad")
      );
    }
  }, [selected, orders]);
  ...
  return (
    <Listbox value={selected} onChange={setSelected}>
      <div className="relative z-40 text-dark dark:text-light">
        ...
        >
          <Listbox.Options className="absolute right-0 mt-2 max-h-48 w-52 overflow-auto rounded-md border border-gray bg-[#1c1d1c] text-base text-light focus:outline-none dark:bg-white dark:text-dark sm:w-44 sm:text-sm">
            {sort.map((item: string, i: number) => (
              <Listbox.Option
                key={i}
                onClick={() => setFilterOrders(orders)}
                className={({ active }) =>
